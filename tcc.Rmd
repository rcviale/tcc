...
kable(readr::read_rds(paste0(path, "table311.rds")), caption = "test")
```

In order to create a synthetic index to represent the "crypto market", the daily market capitalization of each analyzed currency is also obtained from CoinMarketCap and used to create weights for each asset as will be described in Section 4.2.
...
## Synthetic Market Estimates and Weighting Methods
...
Testing the market's volatility as a driver of returns is also one of the goals of this work, hence to obtain an estimate based on 5-minute data, a covariance matrix 
...
kable(readr::read_rds(paste0(path, "tablea13.rds")))
```
...
options(scipen = 10000)
path <- "C:/Users/rodri/OneDrive/Documents/Academics/Trabalho de Conclusão de Curso/tcc/Print/"
kable(readr::read_rds(paste0(path, "tablea11.rds")))
```
...
options(scipen = 10000)
path <- "C:/Users/rodri/OneDrive/Documents/Academics/Trabalho de Conclusão de Curso/tcc/Print/"
kable(readr::read_rds(paste0(path, "tablea12.rds")))
```
